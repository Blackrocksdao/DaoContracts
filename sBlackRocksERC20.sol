...
contract sBlackRocks is IsBR, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgBR public gBR; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked BR", "sBR", 9) ERC20Permit("Staked BR") {
    ...
    function setgBR(address _gBR) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gBR) == address(0), "gBR:  gBR already set");
        require(_gBR != address(0), "gBR:  gBR is not a valid contract");
        gBR = IgBR(_gBR);
    }
    ...
    /**
        @notice increases rBR supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sBR of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sBR
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sBR: insufficient balance");
    }
    ...
    // toG converts an sBR balance to gBR terms. gBR is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gBR.balanceTo(amount);
    }

    // fromG converts a gBR balance to sBR terms. sBR is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gBR.balanceFrom(amount);
    }

    // Staking contract holds excess sBR
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gBR.balanceFrom(IERC20(address(gBR)).totalSupply())).add(
