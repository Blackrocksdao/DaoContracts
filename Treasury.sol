...
contract BlackRocksTreasury is BlackRocksAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for BR
    ...
    function deposit(
        ...
        // mint BR needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        BR.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn BR for reserves
     * @param _amount uint256
     * @param _token address
     */
    ...
    /**
     * @notice mint new BR using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        BR.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or BR from the treasury, using sBR as collateral. This might allow an
     * sBR holder to provide BR liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(BR)) {
            require(permissions[STATUS.BRDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sBR.changeDebt(value, msg.sender, true);
        require(sBR.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(BR)) {
            BR.mint(msg.sender, value);
            BRDebt = BRDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sBR.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with BR
     * @param _amount uint256
     */
    function repayDebtWithBR(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.BRDEBTOR][msg.sender], notApproved);
        BR.burnFrom(msg.sender, _amount);
        sBR.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        BRDebt = BRDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(BR), _amount, _amount);
    }
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SBR) {
            // 9
            sBR = IsBR(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(BR.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns BR valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(BR)).decimals()).div(10**IERC20Metadata(_token).decimals());
